...
| Anamorphic lens support | No open Nuke anamorphic lens prescriptions or knobs found in `flaresim_nuke`. | Ported | Added `Anamorphic Squeeze` to horizontally desqueeze ghost projection for anamorphic plates. This is an artist/camera projection control, not a proprietary cylindrical lens prescription library. |
...
| Per-surface art direction | No open Nuke per-surface art controls found. | Ported | Added `Surface Start`, `Surface Count`, and `Surface Gain` to scale selected surface reflectance before CPU/CUDA tracing. |
...
- 2026-04-29: Added Anamorphic Squeeze. CPU and CUDA ghost projection now share the squeezed sensor-width mapping and invalidate render caches when it changes.
- 2026-04-29: Added per-surface reflectance art direction. CPU and CUDA tracing now respect selected surface gain ranges while keeping base `.lens` files unchanged.
